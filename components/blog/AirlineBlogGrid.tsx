  ...
  return (
    <div className="w-full py-12">
      ...
      <div className="flex justify-center mb-12">
        <div className="relative inline-flex rounded-2xl border border-stone-200 p-2 bg-gradient-to-r from-white via-[#e6faf6] to-white shadow-lg transition-all duration-300">
          {/* Sliding highlight for active button */}
          <div className="absolute inset-0 pointer-events-none flex">
            {filters.map((filter, idx) => (
              <div
                key={filter}
                className={`flex-1 transition-all duration-300 ${activeFilter === filter ? 'bg-emerald-100/80 rounded-xl shadow-md scale-105' : ''}`}
                style={{ zIndex: 0 }}
              />
            ))}
          </div>
          {filters.map((filter) => (
            <button
              ...
              className={`relative px-6 py-2 mx-1 text-base font-semibold rounded-xl transition-all duration-200
                ${activeFilter === filter 
                  ? 'bg-emerald-600 text-white shadow-md scale-105 font-bold' 
                  : 'bg-transparent text-stone-700 hover:bg-emerald-50 hover:text-emerald-700'
                }`}
              style={{ zIndex: 1 }}
            >
              {filter}
