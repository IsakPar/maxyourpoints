-- Fix Newsletter Template Constraints and Update Templates
-- Run this in your Supabase SQL Editor

-- First, drop the existing constraint and recreate it with the new types
ALTER TABLE newsletter_templates DROP CONSTRAINT IF EXISTS newsletter_templates_type_check;

-- Add the new constraint that includes all the types we need
ALTER TABLE newsletter_templates ADD CONSTRAINT newsletter_templates_type_check 
CHECK (type IN ('weekly', 'monthly', 'custom', 'welcome', 'airfare_daily'));

-- Now update the templates with the new beautiful HTML designs

-- Update Subscription Confirmation Template
UPDATE newsletter_templates 
SET 
  name = 'Subscription Confirmation',
  subject = 'Welcome to Max Your Points! ✈️',
  content = 'Welcome to our community of savvy travelers and points enthusiasts!',
...
<html>
  ...
  type = 'welcome'
WHERE name ILIKE '%subscription%' OR name ILIKE '%welcome%' OR type = 'welcome';

-- Update Unsubscribe Confirmation Template  
UPDATE newsletter_templates 
SET 
  name = 'Unsubscribe Confirmation',
  subject = 'Sorry to see you go! 😢 You''ve been unsubscribed',
  content = 'You have been successfully unsubscribed from Max Your Points.',
  html_content = '<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
<body>
  ...
  type = 'custom'
WHERE name ILIKE '%unsubscribe%';

-- Update Weekly Newsletter Template
UPDATE newsletter_templates 
SET 
  name = 'Weekly Newsletter Template',
  subject = 'Max Your Points Weekly: {{date}} Edition ✈️',
  content = 'Your weekly travel tips and deals are here!',
  html_content = '<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
<body>
  ...
  type = 'weekly'
WHERE name ILIKE '%weekly%' OR type = 'weekly';

-- Update or Insert Airfare Deal Template  
UPDATE newsletter_templates 
SET 
  name = 'Airfare Deal of the Day',
  subject = '🔥 Daily Airfare Alert: {{destination}} from ${{price}}!',
  content = 'Amazing airfare deal spotted!',
  html_content = '<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
<body>
  ...
  type = 'airfare_daily'
WHERE name ILIKE '%airfare%' OR name ILIKE '%deal%';

-- Insert Airfare Deal template if it doesn't exist
INSERT INTO newsletter_templates (name, subject, content, html_content, type)
SELECT 'Airfare Deal of the Day', '🔥 Daily Airfare Alert: {{destination}} from ${{price}}!', 'Amazing airfare deal spotted!', 
'<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Airfare Deal Alert</title></head><body>Beautiful airfare template here</body></html>', 'airfare_daily'
WHERE NOT EXISTS (SELECT 1 FROM newsletter_templates WHERE name ILIKE '%airfare%' OR type = 'airfare_daily');

-- Insert subscription template if it doesn't exist  
INSERT INTO newsletter_templates (name, subject, content, html_content, type)
SELECT 'Subscription Confirmation', 'Welcome to Max Your Points! ✈️', 'Welcome message', 
'<!DOCTYPE html><html><head><meta charset="utf-8"><title>Welcome!</title></head><body>Welcome message here</body></html>', 'welcome'
WHERE NOT EXISTS (SELECT 1 FROM newsletter_templates WHERE type = 'welcome');

-- Update timestamps
UPDATE newsletter_templates SET updated_at = NOW() WHERE type IN ('welcome', 'custom', 'weekly', 'airfare_daily'); 